...
function getCityList(){
    //var current_city;
    //!(current_city=Storage.get(Storage.CITY)) ? getLocation() : $('.current_city').text(current_city.cityName);
    //获取城市
    Ajax.custom({
...
function successAfter(res){
    var isSuccess = !!res.data.assetTotalPrice || false;
    loanMain(res.data.busseyType,isSuccess,res.data.id);
}
 ...
 * @param id
 */
function loanMain(type,isSuccess,id){
    if(!type){
        Tools.toast('Result Error!'); return;
    }
    var url = '',
        isMyAsset = $('input[name="type"]').val();
    switch (type){
        case 1:
            url = config.PGaugeResult + '?id=' + id ;
            break;
        case 2:
            url = config.POtherGaugeResult + '?id=' + id;
            break;
        case 3:
            url = config.PAddAnJie + '?id=' + id + '&from_loan=true';
            if(!isSuccess){
                url = config.PGaugeFail + '?id=' + id;
                if(isMyAsset == '2'){
                    url = config.POtherGaugeFail + '?id=' + id;
                }
            }
            break;
        case 4:
            url = config.PApplyLoadAll + '?id=' + id ;
            if(!isSuccess){
                url = config.PGaugeFail + '?id=' + id;
                if(isMyAsset == '2'){
                    url = config.POtherGaugeFail + '?id=' + id;
