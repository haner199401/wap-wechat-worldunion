...
(function(){
    ...
    body.on('click','.checkbox i', function () {
        ...
        $('.checkbox i').each(function () {
            arr.push($(this).hasClass('active') ? 2 : 1);
        });
    ...
    body.on('click','#submit_btn',function(e){
        ...
        Ajax.submitForm({
            ...
            Tools.toast(res.desc || '提交成功',function(){
                location.href =  config.PRentDetail + '?id=' + id;
            });
        });
    });
    ...
    body.on('click','.choose_hht',function(){
        ...
        $('.box,.box-layer').css({
            height:Tools.getDocument().height + 'px',
            display:'block'
        });
        initIScroll();
    });
 ...
 */
function validForm() {
    var decoration = $('input[name="decoration"]'),
        necessarySettings = $('input[name="necessarySettings"]'),
        entrustPrice = $('input[name="entrustPrice"]'),
        house = $('[name="cHouse"]').val();

    if(decoration.val().isEmpty()){
        return '请选择装修情况';
    }

    if(necessarySettings.val().isEmpty()){
        return '请选择配套设施';
    }
 ...
 * 获取配套设施
 */
function getHouseOtherData(){
    Ajax.getDictionary({
        url:config.IDictionary,
        data:{
            flexValuekey:config.dictionary.Entrust_necessarySettings
        },
        renderFor:'rf_list_tmpl',
        renderEle:'#rf_list'
    },function(){
        $('#rf_list a').on('click',function(){
            $('#rf').val($(this).text());
            $('#h_rf').val($(this).attr('data-id'));
        });
    });
